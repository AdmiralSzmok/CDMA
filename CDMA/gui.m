...
% Last Modified by GUIDE v2.5 30-Jun-2014 15:14:20
...
%BBER()
    ...
    a=get(handles.od,'String');
    b=get(handles.do,'String');
    a=str2num(a);
    b=str2num(b);
    enb=1;
    fk= 1;
...
%%
    ...
    figure();
     x=a:b;
    fb= fk./dseq;
    p=10.^(-x./10).*enb.*fk./dseq;
	teo=0.5.*erfc(sqrt(enb*fb*dseq./p));
    
      semilogy(SNR,bufor,'*r',...
                    x ,teo,'green');
    
    title('BER')
    xlabel('SNR [dB]');
    ylabel('BER')
    legend('Symulacja','DSSS Teoria')
                
    %{
for k=a:b
    %k - ca?y zakres SNR
    ebn0 = 10^(k/10);
    eb = (sum(bpsk_mod).^2)./(length(bpsk_mod)/100*nosna);
    n0=eb/ebn0;
    BER(k) = 0.5*erfc(sqrt(eb/n0));
end

figure;
    a=-29:10;
   
    semilogy(a,(BER),'b',SNR,simBER,'r*')
%}
    %{
    ...
    %EbN0=(1./length(a)).*(1./J);
    error = (1/2)*erfc(sqrt(15*1.5.^(EbN0/15*1.3)))-0.001;
    %error = (1/2)*erfc(sqrt(EbN0));
...
% --- Executes during object creation, after setting all properties.
function dzikie_CreateFcn(hObject, ~, handles)
% hObject    handle to dzikie (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
